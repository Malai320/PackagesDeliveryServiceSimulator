/*The program requires 4 inputs from the command line
(number of packages, number of delivery employees, maximum arrival time for each package, maximum wait time for each package).
It then simulates time in seconds and assigns each package to the available employee according to their priority
(red, orange, yellow, green / red - most important, green - least important). If no one is available, the packages
wait in the queue. After the execution, the program will print the report which contains information about
the number of delivery employees, the number of packages, number of packages for each label of priority,
number of packages assigned to each delivery employee, completion time (when all of the packages in the queue
are delivered), average time of the packages spent in the queue, and maximum waiting time of one package in the queue.
*/
